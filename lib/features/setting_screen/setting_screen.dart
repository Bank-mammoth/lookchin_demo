import 'package:flutter/material.dart';
import 'package:go_router/go_router.dart';
import 'package:lookchin_app/constants/constants.dart';
import 'package:lookchin_app/features/auth/auth_screen.dart';
import 'package:lookchin_app/features/setting_screen/models/carditem_mode.dart';
import 'package:lookchin_app/features/setting_screen/widgets/build_card_item.dart';
import 'package:lookchin_app/utils/simple_widgets.dart';
import 'package:shared_preferences/shared_preferences.dart';

class SettingScreen extends StatelessWidget {
  static const routeName = "setting";
  const SettingScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
          backgroundColor: colorWhite,
          title: ZimpleWidgets.zText("Setting",
              fontSize: FontSize.title, useBold: true)),
      body: SingleChildScrollView(
        child: Column(
          children: [
            BuildCardSetting(
              title: "Notifications",
              listCard: [
                CardItem(
                    titlecard: "Alert",
                    icon: const Icon(Icons.notifications_none_outlined),
                    onTap: () {}),
              ],
            ),
            BuildCardSetting(
              title: "User settings",
              listCard: [
                CardItem(
                    titlecard: "Change country",
                    icon: const Icon(Icons.language),
                    onTap: () {}),
                CardItem(
                    titlecard: "Language settings",
                    icon: const Icon(Icons.wordpress_outlined),
                    suffixText: "Thai",
                    onTap: () {}),
                CardItem(
                    titlecard: "Theme",
                    icon: const Icon(Icons.blur_circular_rounded),
                    onTap: () {}),
              ],
            ),
            BuildCardSetting(
              title: "More info and support",
              listCard: [
                CardItem(
                    titlecard: "What's new?",
                    icon: const Icon(Icons.info_outline_rounded),
                    onTap: () {}),
                CardItem(
                    titlecard: "Help",
                    icon: const Icon(Icons.help_outline_rounded),
                    onTap: () {}),
                CardItem(
                    titlecard: "Terms of Use",
                    icon: const Icon(Icons.person_search_rounded),
                    onTap: () {}),
                CardItem(
                    titlecard: "Privacy Policy",
                    icon: const Icon(Icons.privacy_tip_outlined),
                    onTap: () {}),
                CardItem(
                    titlecard: "Location-Base Service Terms",
                    icon: const Icon(Icons.my_location_outlined),
                    onTap: () {}),
                CardItem(
                    titlecard: "Version",
                    icon: const Icon(Icons.file_copy),
                    suffixText: "1.0.0(1)",
                    onTap: () => ZimpleWidgets.zShowDialogOneButton(
                        context: context,
                        title: "You are currently using the \n lastest version",
                        fontSize: FontSize.headline,
                        buttonTitle: "Confirm")),
              ],
            ),
            BuildCardSetting(
              useDivider: false,
              listCard: [
                CardItem(
                    titlecard: "Log out",
                    icon: const Icon(Icons.logout_outlined),
                    onTap: () {
                      ZimpleWidgets.zShowDialogTwoButton(
                          backgroundColor: colorWhite,
                          context: context,
                          title: "Log out",
                          content: "Are tour sure you want to log out?",
                          titleBold: true,
                          fontSize: FontSize.headline,
                          btSaveTitle: "Log out",
                          onSavePressed: () {
                            logOut().then((value) =>
                                context.goNamed(AuthenScreen.routeName));
                          });
                    }),
                CardItem(
                    titlecard: "DeleteAccount",
                    icon: const Icon(Icons.person_off),
                    onTap: () {
                      debugPrint("Delete Account");
                    }),
              ],
            ),
          ],
        ),
      ),
    );
  }

  Future<void> logOut() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    prefs.setBool(Preference.loggedin, false);
  }
}
